...
class Board:
    def __init__(self, position_list=None) -> None:
    ...
    def updatePossibleMovesHumanFormat(self,
        ...
        if self.doubles_turn == True and len(self.unused_dice) > 0:
            ...
            for roll in self.unused_dice:
                self.current_possible_moves.append(
                    Board.returnMovesForDie(roll, self.player, (roll == biggerDie) or (len(self.unused_dice) == 1)))
