    ...
    def on_draw(self):
        ...
        if self.state == "1P_TurnAI":
            ...
            if self.game_settings["AI Lines"] == True:
                Graphics.DrawMoveLines(self.Main_Board.MoveLineData)
        ...
        if self.state == "Splash":
            ...
            if self.step == "main":
                ...
                if clicked_sprite != []:
                    ...
                    self.Main_Board.makeMove((self.Current_Turn.sprite_active.move[0],clicked_sprite[0].pos),1)
                    if self.Main_Board.pip[0] == 0:
                        self.state = "GameOver"
                        self.Game_Winner = 1
                        return
                    roll = Logic.fromMoveToDie(self.Current_Turn.sprite_active.move[0],clicked_sprite[0].pos,self.Current_Turn.unused_dice,1)
            ...
            if self.step == "main":
                ...
                if clicked_sprite != []:
                    arcade.play_sound(Graphics.checker_move,5)
                    self.Main_Board.makeMove((self.Current_Turn.sprite_active.move[0],clicked_sprite[0].pos),2)
                    if self.Main_Board.pip[1] == 0:
                        self.state = "GameOver"
                        self.Game_Winner = 2
                        return
                    roll = Logic.fromMoveToDie(self.Current_Turn.sprite_active.move[0],clicked_sprite[0].pos,self.Current_Turn.unused_dice,0)
                    self.Current_Turn.unused_dice.remove(roll)
                    if len(self.Current_Turn.unused_dice) > 0:
                        self.Current_Turn.updatePossibleMovesHumanFormat(self.Main_Board)
                        self.Current_Turn.formSpriteList(self.Main_Board)
            ...
            if 1025 < x < 1175 and 362 < y < 438:
                ...
                Moves = AI.Main(self.Main_Board,self.Current_Turn,self.game_settings["AI Player"])
                self.aiMoves = Moves
                self.Main_Board.makeMoves(Moves,self.Current_Turn.player,True)
